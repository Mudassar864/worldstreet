...
const SearchResultsBase = ({ searchTerm, id }: SearchResultsProps) => {
  ...
  const renderResultIcon = (result: any) => {
    ...
    const iconClass =
      "mask mask-blob flex h-9 w-9 items-center justify-center bg-[#17161a]/10 text-xs font-bold text-[#17161a] dark:bg-[#17161a]/20";
  ...
  const renderSubMenu = (result: any, i: number) => (
    <div className="bg-muted-50 dark:bg-muted-900 rounded-b-lg border-b border-x border-muted-200 dark:border-muted-800 ml-4">
      {result.subMenu.map((sub: any, j: number) => (
        ...
        >
          {sub.icon && (
            <div className="mask mask-blob flex h-9 w-9 items-center justify-center bg-[#17161a]/10 text-xs font-bold text-[#17161a] dark:bg-[#17161a]/20">
          ...
          <div className="me-2 ms-auto">
            <div className="flex h-7 w-7 items-center justify-center rounded-full bg-[#17161a]/10 opacity-0 transition-opacity duration-300 group-hover:opacity-100">
              <Icon
                ...
                className="h-3 w-3 -translate-x-1 text-[#17161a] transition-transform duration-300 group-hover:translate-x-0"
              />
            </div>
          </div>
        </a>
          ...
          <div className="me-2 ms-auto">
            ...
            >
              <Icon
                ...
                className={`h-3 w-3 text-[#17161a] transition-transform duration-300 ${
