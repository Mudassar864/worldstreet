              ...
              row.push({
                ...
                element: (
                  <Link key={post.id} href={`/blog/post/${post.slug}`}>
                    <Card
                      color={"contrast"}
                      className={`relative w-full h-full p-2 hover:shadow-lg cursor-pointer ${className} hover:border-[#17161a] transition-all duration-300 dark:hover:border-primary-400`}
                    >
                      {header}
                      <div className="p-2">
